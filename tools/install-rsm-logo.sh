...
if [ $# -ne 1 ]; then
  echo "Usage: bash tools/install-rsm-logo.sh /absolute/path/to/RSM_monogram_white_on_black.png"
  exit 1
fi
...
for path in files:
    ...
    # favicon: point each page to the same canonical RSM monogram.
    ...
    # Replace current text-only brand block with logo + wordmark structure.
    if path == Path('music/index.html'):
        ...
        # Album pages use ../../ back to MUSIC TOP.
        ...
        if n == 0:
            # Leave untouched rather than guessing a different structure.
...
# Append only if the RSM brand styles are not already present.
